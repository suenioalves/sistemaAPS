SELECT
    ...
    END AS metodo,
    TO_CHAR(MAX(ta.dt_inicio), 'DD/MM/YYYY') AS dataaplicacao  -- Formato brasileiro com barras
FROM tb_cidadao tc
...
GROUP BY 
    tc.co_seq_cidadao, tc.no_cidadao, tc.dt_nascimento, tm.no_principio_ativo, tm.co_forma_farmaceutica
ORDER BY MAX(ta.dt_inicio) DESC;  -- Ordenação pela data da última aplicação (mais recente primeiro)
