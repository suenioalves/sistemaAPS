-- Criação da tabela para gerenciamento de insulinas no diabetes
...
CREATE TABLE IF NOT EXISTS sistemaaps.tb_hiperdia_dm_insulina (
    ...
    doses_estruturadas JSONB NOT NULL,
    ...
    CONSTRAINT unique_active_insulin_type UNIQUE (codcidadao, tipo_insulina, data_fim) DEFERRABLE INITIALLY DEFERRED
);

-- Função para validar estrutura das doses
CREATE OR REPLACE FUNCTION validar_doses_insulina(doses JSONB, frequencia INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    dose_item JSONB;
    dose_valor INTEGER;
...
BEGIN
    ...
    IF jsonb_array_length(doses) != frequencia THEN
        RETURN FALSE;
    END IF;
    
    -- Validar cada dose individual
    FOR dose_item IN SELECT jsonb_array_elements(doses)
    LOOP
...
CREATE TRIGGER tr_update_insulina_timestamp
    ...
    FOR EACH ROW
    EXECUTE FUNCTION update_insulina_timestamp();
...
CREATE TRIGGER tr_prevent_duplicate_insulin
    ...
    EXECUTE FUNCTION prevent_duplicate_active_insulin();
...
(
    ...
    'Insulina NPH', 
    2, 
    '[{"dose": 12, "horario": "08:00"}, {"dose": 8, "horario": "20:00"}]'::JSONB,
...
(
    ...
    'Insulina Regular', 
    3, 
    '[{"dose": 4, "horario": "08:00"}, {"dose": 6, "horario": "12:00"}, {"dose": 4, "horario": "18:00"}]'::JSONB,
...
SELECT 
    ...
    CONCAT(
        ...
        array_to_string(
            ARRAY(
                SELECT CONCAT((doses->>'dose'), ' U às ', (doses->>'horario'))
                FROM jsonb_array_elements(i.doses_estruturadas) AS doses
            ), ' | '
        )
    ) as resumo_doses
